...
const UnifiedCaseModal: React.FC<CaseDetailPanelProps> = ({ case_, isOpen, onClose, onSave, onDelete }) => {
		...
		if (case_ && isEditing) {
			...
			for (let i = 1; i <= 4; i++) {
				...
				if (method && amount) {
					...
					if (wasCorreted) {
						correctedCase[amountKey] = correctedAmount as any
	...
	useEffect(() => {
		if (existingImmunoRequest) {
			const reactions = existingImmunoRequest.inmunorreacciones.split(',').map(r => r.trim()).filter(r => r)
	...
	const handleCancelEdit = () => {
		setIsEditing(false)
		setEditedCase({})
		setImmunoReactions(existingImmunoRequest ? 
			existingImmunoRequest.inmunorreacciones.split(',').map(r => r.trim()).filter(r => r) : 
			[]
		)
	}
